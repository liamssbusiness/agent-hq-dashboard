...
before a `claude` run the agent's `memory/agents/<agent>/longterm.md` (if any)
is injected into the prompt; after every successful run an episode line is
appended to `memory/episodes/YYYY-MM.jsonl`.
...
that log on startup so state survives restarts.
